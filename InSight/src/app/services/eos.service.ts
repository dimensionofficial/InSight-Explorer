...
import {producersList} from './producers-data';
...
export class EosService {
  ...
  getProducersNew() {
    return from(this.eos.getProducerSchedule({
      json: true,
      code: 'eosio',
      scope: 'eosio',
      table: 'producers',
      ...
      map(result => {
        // @ts-ignore
        if (result.active.producers.length < 10) {
          // @ts-ignore
          result.active.producers = result.active.producers.concat(producersList);
        }
        // @ts-ignore
        result.active.producers.map(row => {
          row.owner = row.owner || row.producer_name;
          row.total_votes = parseFloat(row.total_votes) || 2796623889128686592;
          // console.log('每一行',row);
          }
        );
        // @ts-ignore
        console.log(result.active.producers.length)
        // @ts-ignore
        return result.active.producers.sort((a, b) => b.total_votes - a.total_votes);
      })
    );
  }

...
import {map, share, switchMap, filter, take} from 'rxjs/operators';
...
export class ProducersComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //   share()
    // );
    this.producers$ = this.eosService.getProducersNew().pipe(
      map(producers => {
        const votesToRemove = producers.reduce((acc, cur) => {
          const percentageVotes = cur.total_votes / 69355252837053579264 * 100;
        ...
        return producers.map((producer, index) => {
          ...
          let reward = 0;
          let percentageVotes = producer.total_votes / 69355252837053579264 * 100;
          let percentageVotesRewarded = producer.total_votes / (69355252837053579264 - votesToRemove) * 100;

          if (environment.token === 'DMCT') {
            if (position < 22) {
              reward = 1200;
            } else if (position < 52) {
              reward = 200;
          ...
          if (String(producer.isExist) === 'false') {
            producer.ownerLink = 'gaeatestnet1';
          } else {
            producer.ownerLink = producer.owner;
          }
          return {
            ...producer,
            position: position,

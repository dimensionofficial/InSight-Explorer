...
export class ProducerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.producer$ = combineLatest(
      ...
      this.eosService.getProducersNew(),
      this.name$.pipe(
        switchMap(name => this.eosService.getDeferAccount(name))
      )
    ).pipe(
      map(([name, producers, account]) => {
        ...
        const votesToRemove = producers.reduce((acc, cur) => {
          const percentageVotes = cur.total_votes / 69355252837053579264 * 100; //chainStatus.total_producer_vote_weight
